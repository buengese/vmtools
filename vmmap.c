...
MODULE_DESCRIPTION("vmhax memory mapping module");
MODULE_AUTHOR("bngs");
MODULE_LICENSE("GPL");
...
static int vmreadinit(void)
{
	proc_create("vmmap", 0, 0, &fops);
	KSYMDEF(insert_vm_struct);
	KSYMDEF(vm_area_alloc);
	KSYMDEF(vm_area_free);

	printk("vmmap: initialized\n");
	return 0;
}

static void vmreadexit(void)
{
	remove_proc_entry("vmmap", NULL);
	printk("vmmap: uninitialized\n");
}
...
static void vmmap(ProcessData* data)
{
	...
	if (addr) {
		...
		printk("vmmap: mapping successful!\n");
	}
}
